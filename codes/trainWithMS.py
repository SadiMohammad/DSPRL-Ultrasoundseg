...
class Train(Config):
    ...
    def main(self, model, optimizer, device):
        ...
        datasetTrain = Dataset_ROM(
            imgTrain, maskTrain, self.size, convert='RGB')
        ...
        for epoch in range(self.epochs):
            ...
            for i_train, sample_train in enumerate(tqdm(loaderTrain)):
                ...
                modelBatchLoss = getattr(
                    Loss(trueMasks, preds), self.loss_fn)()
                modelBatchDice = torch.mean(
                ...
                msBatchLoss = getattr(
                    Loss(trueMasks, msPredTensor), self.loss_fn)()
            ...
            model.eval()
            with torch.no_grad():
                epochModelValDice, epochMSMValDice = evalModelwMSM(model, loaderValid, device)
            ...
            print(' ! Epoch Train Dice Coeff: {}'.format(epochModelTrainDice / (i_train + 1)))
            print(' ! Epoch Train Dice Coeff wMS: {}'.format(epochMSMTrainDice / (i_train + 1)))
