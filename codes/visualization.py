...
def out_to_csv(arg):
    ...
    loi = ['Epoch Loss', 'Epoch Train Dice Coeff',
           'Epoch Validation Dice Coeff', 'Best Validation Dice Coeff']
    df = pd.DataFrame()
...
def loss_plot(arg):
    ...
    ax.plot(list(range(0, len(df1['Epoch Validation Dice Coeff']))), df1['Epoch Validation Dice Coeff'], label='DeepLabV3', linewidth=2)
    df2 = pd.read_csv(os.path.join(arg.file_path, 'csv', arg.model2 + '.csv'))
    ax.plot(list(range(0, len(df2['Epoch Validation Dice Coeff']))), df2['Epoch Validation Dice Coeff'], label='UNet', linewidth=2)
