...
class Test(Config):
    ...
    def main(self, model, device):
        ...
        datasetTest = Dataset_ROM(imageFiles, maskFiles, self.size, convert="RGB")
        ...
        for i_test, sample_test in enumerate(tqdm(loaderTest)):
            ...
            mBatch_dice_coeff = torch.mean(Loss(trueMasks, predMasks).dice_coeff())
            tot_dice_coeff += mBatch_dice_coeff.item()
            mBatch_iou = torch.mean(Loss(trueMasks, predMasks).iou_calc())
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load(
        os.path.join(test.checkpointsPath, test.modelName, test.modelWeight)
    )
    model.load_state_dict(checkpoint["model_state_dict"])

    sys.stdout = Logger(os.path.join(test.logOutPath, "{}_test.out".format(time_stamp)))
